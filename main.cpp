...
* On "ALG" function bodies I have commented out print statements to have a clean compilier output
* In implementation 10k-100k had really long compile times, used 1k-10k instead
...
//class is used for calculations
class TimeAlg {
public:
	std::vector <int> n; //where i'll store the total of n 
	std::vector <int> j; //iterations
	std::vector <std::chrono::duration<float>> duration; // creating a vector to store difference from beginning to end
	std::chrono::duration<float> averageTime; //will use this as a sum in loops, then will divide by m after summation
	std::vector <std::chrono::duration<float>> averageTimeN; //creating a vector to store multiple averages depending on n
};
...
int main() {
	...
	*/
	...
	//creating objects to store all data
	TimeAlg timeAlg1;
	TimeAlg timeAlg2;
	TimeAlg timeAlg3;
	...
	for (int j = 1; j <= m; j++) {
		...
		for (int n = 1000; n <= 10000; n = n + 1000) {
			...
			//ALG1
			std::vector<int> B(n);
			//copying contents of A into B based on n size
			for (int j = 0; j < n; j++) {
				B[j] = A[j];
			}
			...
			//creating an object to store information needed for calculations and to keep track in compiler for debugging
			//n is inputs
			//j is iterations (m)
			//duration is for difference in time
			//this gets repeated for the 3 ALG instances
			...
			//ALG2
			//reallocating B with same elements
			for (int j = 0; j < n; j++) {
				B[j] = A[j];
			}
	...
	//creating objects to store averages of previous timeAlg objects
	TimeAlg t1Avg;
	TimeAlg t2Avg;
	TimeAlg t3Avg;
	...
	//In the object t_Avg I have n vector to store n sizes for debugging purposes
	//in every 5 indexes is a new size of n
	//I am just adding +5 so this can be displayed to console
	//0-4 => 1k /  5-10 => 2k / 10-15 => 3k  so on..
	...
	for (int l = 0; l < t1Avg.averageTimeN.size(); l++) {
		std::cout << "AVG TIME for input size " << t1Avg.n[inputSz] << ": " << t1Avg.averageTimeN[l].count() * 1000000 << " microseconds" << "\n";
		inputSz = inputSz + 5;
	}
	...
	for (int l = 0; l < t2Avg.averageTimeN.size(); l++) {
		std::cout << "AVG TIME for input size " << t2Avg.n[inputSz] << ": " << t2Avg.averageTimeN[l].count() * 1000000 << " microseconds" << "\n";
		inputSz = inputSz + 5;
	}
	...
	for (int l = 0; l < t3Avg.averageTimeN.size(); l++) {
		std::cout << "AVG TIME for input size " << t3Avg.n[inputSz] << ": " << t3Avg.averageTimeN[l].count() * 1000000 << " microseconds" << "\n";
		inputSz = inputSz + 5;
	}
	return 0;
}
